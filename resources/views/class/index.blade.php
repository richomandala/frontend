...
<div class="card">
    ...
    <div class="card-body">
        {!! session('response') !!}
        <a href="{{ route('class.create') }}" class="btn btn-primary mb-4">Create</a>
            ...
            <table class="table table-bordered table-hover datatable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item['major'] }}</td>
                        <td class="d-flex">
                            <a href="{{ route('class.show', $item['id']) }}" class="btn btn-success mr-2"
                                title="Detail">
                                Detail
                            </a>
                            <a href="{{ route('class.edit', $item['id']) }}" class="btn btn-primary mr-2"
                                title="Edit">
                                Edit
                            </a>
                            <form id="form_{{ $loop->iteration }}" action="{{ route('class.destroy', $item['id']) }}"
                                method="post">
                                @csrf @method('DELETE')
                            </form>
